...
if __name__ == '__main__':
    ...
    labels = sorted(set([k.split('_')[-1] for k in res.keys()]).union(set(v for v in res.values())))
    ...
    accuracy = f'Out of {len(pred)} annotations, there are {sum(pred==true)} annotation correctly predicted by memad'
    in_details = "\n".join(f'There are {sum(p==t==id for p,t in zip(pred,true))} correct {lang} annotations' for id,lang in id2l.items())
