...
def load(corpus_dir, langs, splits):
    ...
    return pd.concat(split_dfs).set_index("id",
                                          drop=True,
...
def load_split(corpus_dir, langs, split):
    ...
    Languages are filtered based on langs.
    """
    df = pd.read_csv(os.path.join(corpus_dir, split, "utt2label"),
                     sep=" ",
                     names=("id", "label"))
    u2p = pd.read_csv(os.path.join(corpus_dir, split, "utt2path"),
                      sep=" ",
                      names=("id", "path"))
...
def load_all(data_sources, splits, num_processes=os.cpu_count()):
    ...
    if num_processes > 0:
        with multiprocessing.Pool(processes=num_processes) as pool:
            lang_dfs = pool.starmap(load,
                                    ((corpus_dir, langs, splits)
                                     for corpus_dir, langs in data_sources))
    else:
        lang_dfs = (load(corpus_dir, langs, splits)
                    for corpus_dir, langs in data_sources)
...
def verify_integrity(meta, max_threads=None):
    ...
    This function throws an exception if verification fails,
    otherwise completes silently.
    """
    missing_columns = set(REQUIRED_META_COLUMNS) - set(meta.columns)
    assert missing_columns == set(
    ), "{} missing columns in metadata: {}".format(len(missing_columns),
    ...
    if max_threads is None or max_threads > 0:
        with ThreadPoolExecutor(max_workers=max_threads) as pool:
            num_invalid = sum(
                int(not ok)
                for ok in pool.map(os.path.exists, meta.path, chunksize=100))
    else:
