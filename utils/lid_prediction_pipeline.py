...
# Since I had multiple separate scripts with,
# I used a separate configuration file
# to define paths in one place. Replace with hardcoded path strings
# if you want.
...
# This bit I use to confirm that GPU is used instead of CPU.
# The assertion will stop the execution
# if no GPU is found. Computing embeddings with CPU is slow.
...
# These two functions are here because I am not familiar enough
# with Tensorflow logic to be able
# to pass the embedding model 'extractor' as an argument
# instead of global variable. There was
# no trivial answer available just by googling.
def batch_extract_embeddings(x):
    """Convert an audio signal to a logmel spectrogram and
    then into an embedding vector."""
    with tf.device("GPU"):
...
# Predictions are computed for 2 second chunks, now
...
# Write result to an 'utt2lang' file where
# each utterance id is paired with the predicted
...
for ids, corpus in result.groupby("corpus_dir"):
    ...
    # corpus["nb_pred"].to_csv(f"{cdir}/{splits[0]}/utt2lang", sep=" ",
    # header=False)
